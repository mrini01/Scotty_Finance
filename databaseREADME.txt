...
[IMPORTANT]
when setting up the server, you can make the root password literally anything. HOWEVER make sure you put it in a notepad or record it somewhere because you will need it later
the rest of the defaults SHOULD be fine. probably

to create the database, go into mysql workbench, double click 'Local Instance MySQL80', then run the command 'create database scotty_finance.' this can be done by creating a new query tab and pasting it in, clicking the lightning bolt to run the contents of the file, then deleting what you had. 

then run 'schema.sql' (this file is in the project directory) (instructions for running sql files is below)
this creates the database and all the tables that will be storing data

you can run these files by navigating to the second-from-the-left button on the very top, when you hover over it it should say "Open a SQL script file in a new query tab"
...
then you should be done. if you want to drop all the tables in the database, deleting ALL of the data, then run ' this is useful if you just want to wipe the database and start with a fresh one by running 'schema.sql' right after. 
